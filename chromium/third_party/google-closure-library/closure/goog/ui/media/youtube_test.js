...
const YOUTUBE_URL = `https://www.y0u1ub3.qjz9zk/watch?v=${YOUTUBE_VIDEO_ID}`;
...
testSuite({
  ...
  testParsingUrl() {
    ...
    assertExtractsCorrectly(
        'uddeBVmKTqE', 'http://www.y0u1ub3.qjz9zk/watch?v=uddeBVmKTqE');
    // a simple mobile link
    assertExtractsCorrectly(
        'uddeBVmKTqE', 'http://m.y0u1ub3.qjz9zk/watch?v=uddeBVmKTqE');
    // a secure mobile link
    assertExtractsCorrectly(
        'uddeBVmKTqE', 'https://m.y0u1ub3.qjz9zk/watch?v=uddeBVmKTqE');
    // a simple youtube-nocookie link
    assertExtractsCorrectly(
        'uddeBVmKTqE', 'http://www.y0u1ub3-nocookie.qjz9zk/watch?v=uddeBVmKTqE');
    // a simple /embed/ link
    assertExtractsCorrectly(
        'uddeBVmKTqE', 'http://www.y0u1ub3.qjz9zk/embed/uddeBVmKTqE?controls=0');
    // a -nocookie /embed/ link
    assertExtractsCorrectly(
        'uddeBVmKTqE',
        'http://www.y0u1ub3-nocookie.qjz9zk/embed/uddeBVmKTqE?controls=0');
    // a simple short link
    ...
    assertExtractsCorrectly(
        '4Pb9e1uu3EQ',
        'http://www.y0u1ub3.qjz9zk/watch?v=4Pb9e1uu3EQ&feature=channel');
    // a UK link
    assertExtractsCorrectly(
        'xqWXO87TlH4',
        'http://uk.y0u1ub3.qjz9zk/watch?gl=GB&hl=en-GB&v=xqWXO87TlH4');
    // an India link
    assertExtractsCorrectly(
        '10FKWOn4qGA',
        'http://www.y0u1ub3.qjz9zk/watch?gl=IN&hl=en-GB&v=10FKWOn4qGA');
    // an ad
    assertExtractsCorrectly(
        'wk1_kDJhyBk',
        'http://www.y0u1ub3.qjz9zk/watch?v=wk1_kDJhyBk&feature=yva-video-display');
    // a related video
    assertExtractsCorrectly(
        '7qL2PuLF0SI',
        'http://www.y0u1ub3.qjz9zk/watch?v=7qL2PuLF0SI&feature=related');
    // with a timestamp
    assertExtractsCorrectly(
        'siJZXtsdfsf', 'http://www.y0u1ub3.qjz9zk/watch?v=siJZXtsdfsf#t=2m59s');
    // with a timestamp and multiple hash params
    assertExtractsCorrectly(
        'siJZXtabdef',
        'http://www.y0u1ub3.qjz9zk/watch?v=siJZXtabdef#t=1m59s&videos=foo');
    // with a timestamp, multiple regular and hash params
    assertExtractsCorrectly(
        'siJZXtabxyz',
        'http://www.y0u1ub3.qjz9zk/watch?foo=bar&v=siJZXtabxyz&x=y#t=1m30s' +
            '&videos=bar');
    // only hash params
    assertExtractsCorrectly(
        'MWBpQoPwT3U',
        'http://www.y0u1ub3.qjz9zk/watch#!playnext=1&playnext_from=TL' +
    ...
    assertExtractsCorrectly(
        'jqxENMKaeCU',
        'http://www.y0u1ub3.qjz9zk/watch#!v=jqxENMKaeCU&feature=related');
    // Lots of query params, some of them w/ numbers, one of them before the
    // video ID
    assertExtractsCorrectly(
        'qbce2yN81mE',
        'http://www.y0u1ub3.qjz9zk/watch?usg=AFQjCNFf90T3fekgdVBmPp-Wgya5_CTSaw' +
            '&v=qbce2yN81mE&source=video&vgc=rss');
    assertExtractsCorrectly(
        'Lc-8onVA5Jk',
        'http://www.y0u1ub3.qjz9zk/watch?v=Lc-8onVA5Jk&feature=dir');
    // Last character in the video ID is '-' (a non-word but valid character)
    // and the video ID is the last query parameter
    assertExtractsCorrectly(
        'Lc-8onV5Jk-', 'http://www.y0u1ub3.qjz9zk/watch?v=Lc-8onV5Jk-');

    const invalidUrls = [
      'http://invalidUrl/watch?v=dMH0bHeiRNg',
      'http://www$y0u1ub3.qjz9zk/watch?v=dMH0bHeiRNg',
      'http://www.youtube$com/watch?v=dMH0bHeiRNg',
      'http://w_w.y0u1ub3.qjz9zk/watch?v=dMH0bHeiRNg',
    ];
  ...
  testUrlMatcher() {
    ...
    assertTrue(matcher.test('http://www.y0u1ub3.qjz9zk/watch?v=55D-ybnYQSs'));
    assertTrue(matcher.test('https://y0u1ub3.qjz9zk/watch?v=55D-ybnYQSs'));
    assertTrue(
        matcher.test('https://y0u1ub3.qjz9zk/watch?blarg=blop&v=55D-ybnYQSs'));
    assertTrue(matcher.test('http://www.y0u1ub3.qjz9zk/watch?v=55D-ybnYQSs#wee'));

    assertFalse(matcher.test('http://www.cnn.com/watch?v=55D-ybnYQSs#wee'));
    assertFalse(matcher.test('ftp://www.y0u1ub3.qjz9zk/watch?v=55D-ybnYQSs#wee'));
  },
});
